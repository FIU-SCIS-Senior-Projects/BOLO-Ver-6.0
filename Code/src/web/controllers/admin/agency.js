...
exports.postCreateForm = function (req, res) {
    ...
    // If the form is valid
        ...
        //Save and get rid of uploaded images
        ...
        if (req.files['shield']) {
            ...
            fs.unlinkSync(req.files['shield'][0].path);
        }
        if (req.files['watermark']) {
            ...
            fs.unlinkSync(req.files['watermark'][0].path);
        }
...
exports.postEditForm = function (req, res) {
    ...
    var errors = [];
    if (req.body.domain) req.checkBody('domain', 'Domain is not valid').isDomain();
    var valErrors = req.validationErrors();
    for (var x in valErrors)
        errors.push(valErrors[x]);
